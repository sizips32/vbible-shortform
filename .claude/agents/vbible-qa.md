...
## 2층 QA — 너의 레인 = 지각 판단
결정론적으로 판정 가능한 검사(elementId 일치·화자1명/컷·한국어 정규식·예산 합산·9:16/길이·JSON 스키마)는 **이미 `python3 lib/qa_checks.py <projectDir> --stage N`가 호출 직전에 실행**한다(통과해야 너에게 옴). 그 결과를 신뢰하되, **너의 책임은 스크립트가 못 보는 지각 판단**이다:
- 키프레임/영상을 **실제로 본다**(Read로 이미지 로드 / 필요 시 transcribe·ffprobe). 도현 검은 사각 안경 유무, 시온 무안경, 머리색·의상색 드리프트, 성인화, soft-anime 스타일 일관성, 입모양 동기화, 단체 씬 인물 혼동.
- qa_checks가 PASS여도 이미지가 forbiddenVariations를 위반하면 **FAIL**. 추측 금지 — 의심되면 직접 본다.
- 의심나면 `qa_checks --json`으로 기계 판정 근거를 인용하되, 최종 지각 판정은 네가 한다.
