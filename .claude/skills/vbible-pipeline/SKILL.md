...
**실행 모드:** 서브 에이전트 + 오케스트레이터(순차 파이프라인, 비용 게이트). 에이전트 호출은 `Agent` 도구, **항상 `model: "opus"`**, `subagent_type`에 에이전트명.

**상태 헬퍼:** `python3 lib/state.py` (프로젝트 루트에서). state.json 직접 편집 금지 — 모든 전이는 헬퍼로만.
...
- QA: 단계 2·3·5 직후 `vbible-qa` 호출 → FAIL이면 담당 에이전트 수정 후 재검증(이슈 = 단계 미완료).
...
2. `Agent(subagent_type="vbible-qa", model="opus")` — 2단계 체크(화자분리·립싱크 1명/컷·한국어·화자 매칭). FAIL이면 수정·재검증.
...
2. `Agent(subagent_type="vbible-qa", model="opus")` — 3단계 경계 검증(elementId 일치·토킹헤드 존재·9:16·글자 없음·forbiddenVariations). FAIL이면 흔들린 컷 재생성.
3. 사용자가 키프레임 일관성 확정 → set-output 3 → approve 3 → advance.

## 4단계 — prompt (자동, 게이트 없음)
`Agent(subagent_type="vbible-visual-director", model="opus")` — **i2v 씬 컷만** 한국어 프롬프트 설계 → `briefs/prompts.json`(립싱크 컷은 제외, 컷 유형만 표기). set-output 4 → advance.
...
2. `Agent(subagent_type="vbible-qa", model="opus")` — 5단계 검증(한국어 음성 보존·무음 i2v·예산·매핑·9:16/길이). FAIL이면 해당 컷 재렌더.
3. 사용자 승인 → set-output 5 → approve 5 → advance.

## 6단계 — curate (게이트)
오케스트레이터가 clips.json 결과를 사용자에게 제시, 컷별 채택/재생성 선별 → `briefs/selects.md`. 승인 → set-output 6 → approve 6 → advance.

## 7단계 — post (자동)
1. **후반 체크리스트**: 합성 도구별 안내. 자막·BGM·내레이션·로고·성경구절·전환은 **NLE/합성 단계**(CapCut: `codex/capcut` 브랜치, 또는 hyperframes). 자막은 한국어로 NLE에서만(영상 내 생성 금지). 9:16, 60~90초, 컷 순서 = selects.md.
2. **업로드 메타**: 제목(대본 제목/주제에서 도출), 설명, 고정 해시태그(#VBibleForStudents #학생바이블 #바이블칼리지 #성경공부 #기독교쇼츠 #BibleShorts), 오프닝/엔딩 고정 문구(사용자 대본이 별도 지정 시 그것 우선).
3. `projects/<epSlug>/briefs/post-checklist.md` Write. set-output 7 → advance(완료). 사용 크레딧·다음 화 안내.
